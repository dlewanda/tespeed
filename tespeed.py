...
class TeSpeed:
    ...
    def __init__(self, server = "", numTop = 0, servercount = 1, store = False, suppress = False, unit = False, duration = 5, iterations = 0):
        ...
        self.num_servers=int(servercount);
        self.servers=[]
        if server != "":
            self.servers=[server]
        ...
        self.print_debug("self.duration = %d, self.iterations = %d\n" % (self.duration, self.iterations))

        if store:
            self.print_debug("Printing CSV formated results to STDOUT.\n")
...
#            1024*1024*2, 1024*1024*2]
    ...
    # Finding servers with lowest latency
        self.print_debug("Testing latency...\n")
        po = []
        for server in servers:
            ...
            if now == -1 or now == 0:
                continue
            self.print_debug("%0.0f ms latency for %s (%s, %s, %s) [%0.2f km]\n" % 
    ...
    # Receiving status update from download thread
        ...
        if num==0 or down >= total_size*th:
            ...
            self.print_debug("Downloaded %d of %d bytes (%0.2f%%) in %d threads\r" %
    ...
    def ChunkRead(self, response, num, th, d, w=0, chunk_size=10240, report_hook=None):
        #self.print_debug("Thread num %d %d %d starting to report\n" % (th, num, d))
        if(w==1):
            return [0,0,0]
        ...
        while 1:
            chunk=0
            if start == 0:
                #self.print_debug("Started receiving data\n")
    ...
    def AsyncGet(self, conn, uri, num, th, d):
        ...
        #except urllib2.URLError, e:
        #    self.print_debug("Failed downloading.\n")
        except:
            self.print_debug('                                                                                           \r')
            self.print_debug("Failed downloading.\n")
            conn.send([0, 0, False])
    ...
    def AsyncPost(self, conn, uri, num, th, d):
        ...
        #except urllib2.URLError, e:
        #    self.print_debug("Failed uploading.\n")
        except:
            self.print_debug('                                                                                           \r')
            self.print_debug("Failed uploading.\n")
            conn.send([0, 0, False])
    ...
    # Load the configuration file
        self.print_debug("Loading speedtest configuration...\n")
        ...
        self.print_debug("IP: %s; Lat: %f; Lon: %f; ISP: %s\n" % (ip, lat, lon, isp))
    ...
    # Load server list
        self.print_debug("Loading server list...\n")
    ...
    def FindBestServer(self):
        self.print_debug("Looking for closest and best server...\n")
    ...
    # Testing upload speed
        ...
        for i in range(0, len(self.upSizes)):
            ...
            if len(data) == 0 or self.upSizes[i] != self.upSizes[i-1]:
                #self.print_debug("Generating new string to upload. Length: %d\n" % (self.upSizes[i]))
            ...
            speed=size/took
            self.print_debug("Upload size: %0.2f MiB; Uploaded in %0.2f s\n" % 
                (size, took))
            self.print_debug("\033[92mUpload speed: %0.2f %s/s\033[0m\n" % 
                (speed, self.units))
            
            if self.up_speed<speed:
                self.up_speed=speed

            self.print_debug("Upload: i = %d, self.iterations = %d\n" % (i, self.iterations))
            if took>self.duration:
                break
                
        #self.print_debug("Upload size: %0.2f MiB; Uploaded in %0.2f s\n" % (self.SpeedConversion(sizes), took))
        #self.print_debug("Upload speed: %0.2f MiB/s\n" % (self.SpeedConversion(sizes)/took))
    ...
    # Testing download speed
        ...
        for i in range(0, len(self.downList)):
            ...
            size=self.SpeedConversion(sizes)
            speed=size/took
            self.print_debug("Download size: %0.2f MiB; Downloaded in %0.2f s\n" % 
                (size, took))
            self.print_debug("\033[91mDownload speed: %0.2f %s/s\033[0m\n" % 
                (speed, self.units))
            self.print_debug("Download: i = %d, self.iterations = %d\n" % (i, self.iterations))
    ...
    def TestSpeed(self):
        ...
        if self.server == '':
            ...
            if self.num_servers>1:
                #self.print_debug("num_servers = " % self.num_servers)
...
def main(args):
    ...
    if args.listservers!=True and args.server=='' and args.store!=True:
        TeSpeed.print_debug("Getting ready. Use parameter -h or --help to see available features.\n")
    else:
        TeSpeed.print_debug("Getting ready\n")
    try:
